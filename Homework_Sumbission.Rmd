...
My partner's main suggestion was that I could be more concise. They prefer more in-code annotations and suggest they work better than wordy explanations. I agree that in some cases replacing a lengthier description with an annotation would have benefitted my code. In other cases, however, the complexity of the statistical analysis leads me to prefer more full exposition of that the code is doing.
...
Throughout this I learned a great deal about writing code for other people, including my future-selves. Learning about `renv()` was extremely helpful for understanding why my old projects would stop working when re-loaded and for protecting those to come.

I learned about making and maintaining a GitHub Repository. By setting this up in the first instance, this enables interested readers to audit my commits and see the process of constructing my code, as well as the finished product. Unfortunately I had to delete my initial repository which showed edits from the very beginning due to accidentally "doxxing" myself. This resulted from having my anonymous GitHub account paired with the RStudio terminal but my usual GitHub account paired with Git Bash, from which the RStudio Git tab runs. I learned to check which user is signed in to Git Bash before pushing sensitive files.
